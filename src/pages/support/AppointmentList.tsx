...
interface Appointment {
  ...
  status: 'pending_confirmation' | 'confirmed' | 'waiting_customer' | 'completed';
  isPinned?: boolean;
}
...
export default function AppointmentList() {
  ...
  const pinnedAppts = Object.values(monthAppts)
    .flat()
    .filter((a) => a.isPinned)
    .sort((a, b) => a.appointmentDate.localeCompare(b.appointmentDate) || a.appointmentSlot.localeCompare(b.appointmentSlot));
  ...
  for (let d = 1; d <= daysInMonth; d++)
    cells.push({ day: d, current: true, dateKey: toDateKey(curYear, curMonth, d) });
  ...
  const selectedAppts = selectedDate
    ? apptsByDate(selectedDate).sort((a, b) => a.appointmentSlot.localeCompare(b.appointmentSlot))
    : [];

  const statusPill = (status: Appointment['status']) => {
    const base = 'w-fit rounded-full px-2.5 py-0.5 text-[10px] font-medium';
    if (status === 'confirmed') return cn(base, 'bg-green-100 text-green-800 dark:bg-green-500/10 dark:text-green-400');
    if (status === 'pending_confirmation') return cn(base, 'bg-amber-100 text-amber-800 dark:bg-amber-500/10 dark:text-amber-400');
    if (status === 'waiting_customer') return cn(base, 'bg-blue-100 text-blue-800 dark:bg-blue-500/10 dark:text-blue-400');
    return cn(base, 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200');
  };
  ...
  useEffect(() => {
    ...
    const fetchMonth = async () => {
      try {
        ...
        const { data } = await apiClient.get('/appointments', { params: { month } });
  ...
  useEffect(() => {
    const pinned = Object.values(monthAppts).flat().filter((a) => a.isPinned).map((a) => a._id);
    ...
    <div className="flex h-full flex-col overflow-hidden p-3">
      ...
      <div className="mb-2 flex flex-shrink-0 flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <h1 className="text-xl font-black text-gray-950 dark:text-white">Quản lý lịch hẹn</h1>
          <p className="text-[11px] text-gray-500 dark:text-gray-400">
            Click vào ngày có lịch để xem chi tiết · Ghim tối đa {MAX_PINS} lịch quan trọng
          </p>
        </div>
      ...
      <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[2fr_1fr_1fr]">
        ...
        <div className="relative overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          {loadingMonth && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/60 dark:bg-black/60">
              <div className="text-sm font-medium text-gray-700">Đang tải lịch...</div>
            </div>
          )}
          <div className="grid grid-cols-7 border-b border-gray-100 dark:border-gray-800">
            {WEEKDAYS.map((d) => (
              <div key={d} className="py-2.5 text-center text-xs font-medium text-white">{d}</div>
            ))}
          </div>
          <div className="grid grid-cols-7">
            {cells.map((cell, i) => {
              ...
              const isSel     = !!(cell.dateKey && cell.dateKey === selectedDate && !isTodayCell);
              ...
              return (
                  <div
                    ...
                    className={cn(
                      ...
                      cell.current ? "cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-600/20" : "cursor-default",
                      isTodayOnly && "bg-orange-100 ring-2 ring-orange-500 dark:bg-orange-600/25",
                      isSelectedOnly && "bg-orange-100 ring-1 ring-orange-500 dark:bg-orange-600/25",
                      isTodayAndSelected && "bg-orange-100 ring-2 ring-orange-500 dark:bg-orange-600/25"
                    )}>
                  <div className={cn(
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-[3px]">
                        {appts.slice(0, 5).map((a) => (
                          <span
                            ...
                            style={{ background: a.isPinned ? "#f97316" : a.status === "confirmed" ? "#16a34a" : "#d97706" }}
                          />
                        ))}
                      </div>
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          {!selectedDate ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 p-6 text-center">
              ...
              <p className="text-xs text-gray-400">Click vào ngày có lịch để xem chi tiết</p>
            </div>
                  ...
                  <div
                    ...
                    <div className="mb-1.5 flex items-center justify-between gap-2">
                      <span className={cn(
                        ...
                        isPinned ? "text-amber-700 dark:text-amber-400" : "text-orange-600 dark:text-orange-400"
                      )}>
                        🕐 {a.appointmentSlot}
                      </span>
                      {isPinned && (
                    ...
                    <div className="text-sm font-semibold text-gray-950 dark:text-white">{a.customerName}</div>
                    <div className="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">
                      {a.petName} ({petTypeLabel(a.petType)}) · {a.serviceType}
                    </div>
                    <div className="mt-2 flex items-center gap-2">
                      <span className={statusPill(a.status)}>
                        {a.status === 'confirmed' ? 'Đã xác nhận' : a.status === 'pending_confirmation' ? 'Chờ xác nhận' : a.status === 'waiting_customer' ? 'Chờ khách đến' : 'Đã xong'}
                      </span>
                      <button
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex flex-shrink-0 items-center gap-2 border-b border-gray-100 px-4 py-2.5 dark:border-gray-800">
            <span className="flex-1 text-xs font-bold text-gray-950 dark:text-white">Lịch đã ghim</span>
            <span className={cn(
          ...
          {pinnedAppts.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 p-6 text-center">
              <span className="text-2xl">📌</span>
              <p className="text-xs text-gray-400">Chưa có lịch nào được ghim</p>
            </div>
          ) : (
            <>
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {pinnedAppts.map((a) => (
                  <div
                    ...
                    <div className="text-[11px] text-gray-400 dark:text-gray-500">{a.serviceType}</div>
                    <div className="mt-2">
                      <span className={statusPill(a.status)}>
                        {a.status === 'confirmed' ? 'Đã xác nhận' : a.status === 'pending_confirmation' ? 'Chờ xác nhận' : a.status === 'waiting_customer' ? 'Chờ khách đến' : 'Đã xong'}
                      </span>

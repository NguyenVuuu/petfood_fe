...
const highlights = [
  { icon: Coins, labelKey: "pawmart.rewards.coin", color: "bg-amber-100 text-amber-700" },
  { icon: Ticket, labelKey: "pawmart.rewards.coupon", color: "bg-orange-100 text-orange-700" },
  { icon: Gift, labelKey: "pawmart.rewards.spin", color: "bg-rose-100 text-rose-700" },
  { icon: Truck, labelKey: "pawmart.rewards.freeship", color: "bg-emerald-100 text-emerald-700" },
];
        ...
        <div className="relative overflow-hidden rounded-[calc(2rem-1px)] bg-gradient-to-br from-orange-50 via-white to-amber-50 p-6 dark:from-gray-950 dark:via-gray-900 dark:to-amber-950/30 md:p-8 lg:p-10">
          ...
          <div className="relative grid items-center gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {highlights.map((item) => {
                  ...
                  return (
                    <div key={item.labelKey} className="rounded-2xl border border-white/70 bg-white/75 p-3 shadow-sm backdrop-blur dark:border-gray-800 dark:bg-gray-900/70">
                      <div className={`mb-2 flex h-10 w-10 items-center justify-center rounded-xl ${item.color}`}>
                        <Icon size={18} />
                      </div>
                      <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{t(item.labelKey)}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="relative">
              ...
              >
                <p className="text-xs font-semibold text-gray-500">{t("pawmart.rewards.today")}</p>
                <p className="text-sm font-black text-orange-600">{t("pawmart.rewards.todayPrizes")}</p>
              </motion.div>
            </div>
          </div>
        </div>

...
import { useExchangeReward, useMyRewards, useRewardShop } from "@/hooks/useRewards";
  ...
  return (
    <RewardShell>
      <div className="flex flex-col gap-4 rounded-[2.25rem] bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-500 p-6 text-white shadow-xl md:flex-row md:items-center md:justify-between md:p-8">
        <div>
          <p className="inline-flex rounded-full bg-white/20 px-4 py-1 text-sm font-bold">Reward Shop</p>
          <h1 className="mt-3 text-3xl font-black md:text-4xl">{t("pawmart.rewardsPages.rewardShop")}</h1>
          <p className="mt-2 max-w-2xl text-white/85">{t("pawmart.rewardsPages.shopDesc")}</p>
        </div>
        <CoinBadge coins={reward?.coinBalance ?? 0} className="bg-white text-emerald-700 ring-white/50" />
      </div>
      ...
      {isError ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-72 animate-pulse rounded-3xl bg-gray-100 dark:bg-gray-800" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <EmptyState
      ...
      <Modal isOpen={successOpen} onClose={() => setSuccessOpen(false)} title={t("pawmart.rewardsPages.exchangeSuccess")}>
        <div className="text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-600">
            <Ticket size={30} />
          </div>
          <p className="text-gray-500 dark:text-gray-400">{t("pawmart.rewardsPages.couponAdded")}</p>
          <Button className="mt-5" onClick={() => setSuccessOpen(false)}>{t("pawmart.rewardsPages.close")}</Button>
        </div>
      </Modal>
    </RewardShell>
  );
